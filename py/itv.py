...
def modify_and_check_urls(url):
    parsed_url = urlparse(url)
    base_url = parsed_url.scheme + "://" + parsed_url.netloc
    ip_end = "/iptv/live/1000.json?key=txiptv"
    modified_urls = []
    for i in range(1, 256):
        modified_ip = f"{parsed_url.hostname[:-1]}{i}"
        modified_url = f"{base_url}{modified_ip}{parsed_url.port}{ip_end}"
